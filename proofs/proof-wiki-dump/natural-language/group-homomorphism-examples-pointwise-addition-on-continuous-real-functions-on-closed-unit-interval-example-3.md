...
Source: https://proofwiki.org/wiki/Group_Homomorphism/Examples/Pointwise_Addition_on_Continuous_Real_Functions_on_Closed_Unit_Interval/Example_3
...
\(\ds \int_0^1 \map {\paren {f + g} } x \rd x\)
...
\(\ds \int_0^1 \paren {\map f x + \map g x} \rd x\)
...
\(\ds \int_0^1 \map f x \rd x + \int_0^1 \map g x \rd x\)
