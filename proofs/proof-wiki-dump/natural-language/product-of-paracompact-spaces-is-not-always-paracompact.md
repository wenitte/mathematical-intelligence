...
Source: https://proofwiki.org/wiki/Product_of_Paracompact_Spaces_is_not_always_Paracompact
...
Let each of $\struct {S_\alpha, \tau_\alpha}$ be a paracompact space.

Then it is not necessarily the case that $\struct {S, \tau}$ is also paracompact space.


Proof
Let $T$ be the Sorgenfrey line.
Let $T' = T \times T$ be Sorgenfrey's half-open square topology.
From Sorgenfrey Line is Paracompact Space, $T$ is a paracompact space.
From Sorgenfrey's Half-Open Square Topology is Not Paracompact Space, $T'$ is not a paracompact space.
Hence the result.
$\blacksquare$
