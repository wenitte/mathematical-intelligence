...
Source: https://proofwiki.org/wiki/Primitive_of_Power_of_x_by_Arccotangent_of_x_over_a
...
\(\ds \int a^m \paren {\dfrac x a}^m \arccot \frac x a \rd x\)
...
\(\ds a^m \int \paren {\dfrac x a}^m \arccot \frac x a \rd x\)
...
\(\ds a^m \paren {\dfrac 1 {1 / a} \paren {\frac 1 {m + 1} \paren {\dfrac x a}^{m + 1} \arccot \frac x a + \frac 1 {m + 1} \int \paren {\dfrac x a}^{m + 1} \frac {\d x} {\paren {\dfrac x a}^2 + 1} } }\)
...
Sources
1968: Murray R. Spiegel: Mathematical Handbook of Formulas and Tables ... (previous) ... (next): $\S 14$: Integrals involving Inverse Trigonometric Functions: $14.506$
