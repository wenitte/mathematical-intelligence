...
Source: https://proofwiki.org/wiki/Mapping_whose_Image_of_Natural_Number_n_is_Subset_of_Image_of_Successor/Corollary
...
$\forall n < k: \map f n \subsetneqq \map f k$
...
$\map f k \subsetneqq \map f {k^+}$
That is:

$\forall n \le k: \map f n \subsetneqq \map f {k^+}$
That is:
...
$\forall n, m \in N: n \le m \implies \map f n \subsetneqq \map f m$
$\blacksquare$


Sources
2010: Raymond M. Smullyan and Melvin Fitting: Set Theory and the Continuum Problem (revised ed.) ... (previous) ... (next): Chapter $3$: The Natural Numbers: $\S 5$ Applications to natural numbers: Exercise $5.2 \ \text{(b)}$
