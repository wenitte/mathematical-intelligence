...
Source: https://proofwiki.org/wiki/Pointwise_Infimum_of_Stopping_Times_is_Stopping_Time
...
$\map T \omega \le t$ if and only if $\map {T_n} \omega \le t$ for some $n \in \N$.
That is: 
...
for each $n \in \N$.
Then, since $\FF_t$ is a $\sigma$-algebra and $\sigma$-algebras are closed under countable union, we have: 
