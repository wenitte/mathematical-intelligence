...
Source: https://proofwiki.org/wiki/User:Leigh.Samphier/Topology/Equivalence_of_Definitions_of_Lattice_Isomorphism
...
Then $\phi: L_1 \to L_2$ is a lattice isomorphism if and only if $\phi : A_1 \to A_2$ is a bijection.


Definition 2
Let $\phi: A_1 \to A_2$ be a mapping.

Then $\phi : L_1 \to L_2$ is a lattice isomorphism if and only if $\phi : \struct{A_1, \preceq_1} \to \struct{A_2, \preceq_2}$ is an order isomorphism.
...
$\phi : \struct{A_1, \preceq_1} \to \struct{A_2, \preceq_2}$ is order-preserving
